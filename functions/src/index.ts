...
export const calculateHealthScore = functions.https.onRequest(async (req, res) => {
  setCorsHeaders(res);
  ...
  try {
    ...
    else if (activityLevel === 'light') score -= 10;
    else if (activityLevel === 'very-active') score += 10;
    ...
    if (exerciseFreq === 'Never') score -= 15;
    else if (exerciseFreq === 'Rarely') score -= 10;
    else if (exerciseFreq === '3-4 times per week') score += 5;
    else if (exerciseFreq === 'Daily') score += 10;
    
    if (sleepHours < 6 || sleepHours > 9) score -= 10;
    else if (sleepHours >= 7 && sleepHours <= 8) score += 5;
    
    if (dietQuality === 'Poor (fast food, processed foods)') score -= 15;
    else if (dietQuality === 'Average (mixed healthy and unhealthy)') score -= 5;
    else if (dietQuality === 'Excellent (well-balanced, nutritious)') score += 10;
    
    if (stressLevel === 'Very High' || stressLevel === 'High') score -= 10;
    else if (stressLevel === 'Very Low' || stressLevel === 'Low') score += 5;
    ...
    const assessmentData = {
      userId: 'test-user', // Use default for testing
      score,
      recommendations,
      personalInfo,
      healthHistory,
...
export const getHealthHistory = functions.https.onRequest(async (req, res) => {
  setCorsHeaders(res);
...
export const sendHealthEmail = functions.https.onRequest(async (req, res) => {
  setCorsHeaders(res);
  ...
  try {
    ...
    const response = await axios.post(
      ...
      {
        service_id: emailjsConfig?.service_id || 'service_eb1d0iv',
        template_id: emailjsConfig?.template_id || 'template_42an91c',
        user_id: emailjsConfig?.user_id || '1U6m7-TUFRxv4HcfP',
        template_params: {
